...
class Test_Entry_resources(unittest.TestCase):
    ...
    def test_get_single_entry(self):
        ...
        token = json.loads(res.data.decode('utf-8'))[self.key]
        headers = {'Authorization': 'Bearer {}'.format(token)}
        self.tester.post(self.add_entry, headers=headers,data = json.dumps(self.new_entry),content_type = 'application/json')
    ...
    def test_update_entry(self):
        ...
        res = self.tester.put(self.edit_entry, headers=headers,data = data,content_type = 'application/json')
        self.assertEqual(res.status_code,200)
    ...
    def test_delete_entry(self):
        ...
        headers = {'Authorization': 'Bearer {}'.format(token)}
        data = json.dumps(self.new_entry)
        self.tester.post(self.add_entry, headers=headers,data = data,content_type = 'application/json')
        res = self.tester.delete(self.delete_entry, headers=headers,content_type = 'application/json')
    ...
    def test_using_wrong_url(self):
        ...
        headers = {'Authorization': 'Bearer {}'.format(token)}
        data = json.dumps(self.new_entry)
        res = self.tester.post('\mydiary\v1\andela', headers=headers,data = data,content_type = 'application/json')
        self.assertEqual(res.status_code, 404)
        res = self.tester.post('\mydiary\v1\entries\hi', headers=headers,data = data,content_type = 'application/json')
        self.assertEqual(res.status_code, 404)
        ClearClass().drop_tables()
        ClearClass().create_table()
